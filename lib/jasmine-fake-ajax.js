(function($, jasmine) {
  ...
  function callAvailableCallbackHandlers(real, fake) {
    ...
    if (fake.error) {
      if (_.isFunction(fake.error)) {
        real.error.call(real.context, fake.error.xhr)
      } else if (_.isArray(fake.error)) {
        if (!_.isArray(real.error)) {
          logAndThrow("Fake error is an array but real error is not for url '" + real.url + "'.")
        }
        ...
        logAndThrow("Fake error must be a function or an array of functions for url '" + real.url + "'.")
      }
    }
