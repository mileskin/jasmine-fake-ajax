(function($, jasmine) {
  ...
  function callAvailableCallbackHandlers(real, fake) {
    function checkPreCondition() {
      if (!fake.success && !fake.error && !fake.successData && !fake.errorMessage) {
        logAndThrow("One of success, error, successData or errorMessage must be defined for url '" + real.url + "'.")
      }
      ...
      if ((fake.error || fake.errorMessage) && !real.error) {
    ...
    function callAvailableHandlers() {
      function handleSuccessCallback() {
        if (_.isFunction(real.success)) {
          if (!_.isObject(fake.success) || _.isArray(fake.success)) {
            logAndThrow("Fake error must be an (success arguments) object when real error is a function for url '" + real.url + "'.")
          }
          real.success.call(real.context, fake.successData)
        } else if (_.isArray(real.success)) {
          ...
          logAndThrow("Real success must be a function or an array of functions for url '" + real.url + "'.")
        }
      }

      function handleErrorCallback() {
        if (_.isFunction(real.error)) {
          if (!_.isObject(fake.error) || _.isArray(fake.error)) {
            logAndThrow("Fake error must be an (error arguments) object when real error is a function for url '" + real.url + "'.")
          }
          real.error.call(real.context, fake.error.xhr)
        } else if (_.isArray(real.error)) {
          if (!_.isArray(fake.error)) {
            logAndThrow("Real error is an array but fake error is not for url '" + real.url + "'.")
          }
          if (!_.isEqual(fake.error.length, real.error.length)) {
            logAndThrow("Fake error has " + fake.error.length + " items but real error has " + real.error.length + " items for url '" + real.url + "'.")
          }
          _.each(_.zip(real.error, fake.error), function(realAndFakeError) {
            realAndFakeError[0].call(real.context, realAndFakeError[1].xhr)
          })
        } else {
          logAndThrow("Real error must be a function or an array of functions for url '" + real.url + "'.")
        }
      }
